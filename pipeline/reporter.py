...
def generate_html_report(
    ...
    <style>
        ...
        .meta-card {{ background: #f8f9fa; padding: 16px; border-radius: 8px; border-left: 4px solid #e94560; }}
        .meta-card strong {{ display: block; font-size: 0.85em; color: #666; }}
        .meta-card span {{ font-size: 1.4em; font-weight: bold; }}
        .ai-box {{ background: #f0f4ff; border: 1px solid #c7d7fe; border-radius: 8px;
                   padding: 20px; margin: 20px 0; white-space: pre-wrap; line-height: 1.6; }}
        table {{ width: 100%; border-collapse: collapse; margin: 16px 0; }}
        th {{ background: #1a1a2e; color: white; padding: 10px; text-align: left; }}
        td {{ padding: 8px 10px; border-bottom: 1px solid #eee; }}
        tr:nth-child(even) {{ background: #f8f9fa; }}
        .footer {{ margin-top: 60px; color: #999; font-size: 0.85em; text-align: center; }}
    </style></head><body>
    """)
    ...
    sections.append(
        '<div class="footer">Generated by Data Suite · Powered by Claude</div>'
    )
